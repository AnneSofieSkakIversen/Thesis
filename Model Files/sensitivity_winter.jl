...
begin
    ...
    S2 = setdiff([s for s in 1:1:364],[s for s in 60:1:(364-61)]) # oin([s for s in 1:1:60], [s for s in (364-60):1:364]) # choose which Prosumer scenarios to use in scenario generation
...
begin
    ...
    risktype = "CVaR"    # "slack" "no_risk" "CC" "JCC" "HJCC" "CVaR" "HCVaR"
...
folder = "Winter_Behavior\\"
plot_TCL(my_results)#png(folder*risktype*"_TCL_dynamics.png") ;
plot_EV_dynamics(my_results) #png(folder*risktype*"_EV_dynamics.png") ;

stacked_reserve_bid(my_results,data["C"]) #png(folder*risktype*"_ALL_reserve_bid.png") ;
stacked_reserve_bid(my_results,data["F"]) #png(folder*risktype*"_TCL_reserve_bid.png") ;
stacked_reserve_bid(my_results,data["E"]) #png(folder*risktype*"_EV_reserve_bid.png") ;


stacked_activation(my_results) #png(folder*risktype*"_bid_activation.png") ;
#stacked_bar_q(my_results)
stacked_bar_q_mean(my_results)# png(folder*risktype*"_missing_delivery.png") ;
