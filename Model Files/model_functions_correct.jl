...
### CVaR chance constraints for Ancillary markets
    ...
    @constraint(Coop_model, CVaR_TCL_up2, η_TCL_up + 1/(1 - epsilon)*sum(pi_s[s]*delta_TCL_up[s] for s in S) <= epsilon*sum(pi_s[s]*(p_c_mFRR[f,t] + FCRN_accept[f,t,s]*p_c_FCRN[f,t] + FCRD_accept[f,t,s]*p_c_FCRD[f,t] + p_c_FFR[f,t]) for f in F,t in T,s in S))
    @constraint(Coop_model, CVaR_TCL_down2, η_TCL_down + 1/(1 - epsilon)*sum(pi_s[s]*delta_TCL_down[s] for s in S) <= epsilon*sum(pi_s[s]*(FCRN_accept[f,t,s]*p_c_FCRN[f,t]) for f in F,t in T,s in S))
...
### TCL ### p_c_grid[f,t,s] = p[f,t,s] - d_TCL[f,t,s]
@constraint(Coop_model, TCL_balance[f in F, t in T, s in S], p[f,t,s] == - PV[t,s]*PV_peak[f]*n_demand[f] + D_household[f,t,s] + D_TCL[f,t,s] + d_TCL[f,t,s]) #flexible prosumer energy balance
@constraint(Coop_model, TCL_ancillary[f in F, t in T, s in S], d_TCL[f,t,s] == - p_c_FCRN_up[f,t,s] + p_c_FCRN_down[f,t,s] - p_c_mFRR_up[f,t,s] - p_c_FCRD_up[f,t,s] - p_c_FFR_up[f,t,s])
@constraint(Coop_model, TCL_demand[f in F,t in T,s in S], T_TCL[f,t,s] == (t>1 ? T_TCL[f,t-1,s] : T_TCL[f,24,s]) - 1/(R_th[f]*C_th[f])*((t>1 ? T_TCL[f,t-1,s] : T_TCL[f,24,s])-T_a[t,s]) + η_TCL/C_th[f]*(D_TCL[f,t,s] + d_TCL[f,t,s])) #TCL adjusted demand 
@constraint(Coop_model, d_TCL_capacity[f in F,t in T,s in S],  D_TCL[f,t,s] + d_TCL[f,t,s] <= Pmax_TCL)
@constraint(Coop_model, d_TCL_capacity[f in F,t in T,s in S], -D_TCL[f,t,s] <= d_TCL[f,t,s])
@constraint(Coop_model, d_TCL_grid_relation[f in F,t in T,s in S], p_c_grid[f,t,s] = p[f,t,s] - d_TCL[f,t,s])
